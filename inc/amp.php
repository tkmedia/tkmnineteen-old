 ...
 */
function tkmnineteen_amp_class( $default, $active, $state ) {
	$output = '';
	if( tkmnineteen_is_amp() ) {
		$output .= sprintf(
			' [class]="%s"',
			esc_attr(
				sprintf(
 ...
 */
function tkmnineteen_amp_toggle( $state = '', $disable = array() ) {
	if( ! tkmnineteen_is_amp() )
		return;
 ...
 */
function tkmnineteen_amp_nav_dropdown( $theme_location = false, $depth = 0 ) {
	...
	return tkmnineteen_amp_toggle( $key ) . tkmnineteen_amp_class( 'submenu-expand', 'expanded', $key );
}
