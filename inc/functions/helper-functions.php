...
global $wp_embed;
add_filter( 'tkmnineteen_the_content', array( $wp_embed, 'run_shortcode' ), 8 );
add_filter( 'tkmnineteen_the_content', array( $wp_embed, 'autoembed'     ), 8 );
add_filter( 'tkmnineteen_the_content', 'wptexturize'        );
add_filter( 'tkmnineteen_the_content', 'convert_chars'      );
add_filter( 'tkmnineteen_the_content', 'wpautop'            );
add_filter( 'tkmnineteen_the_content', 'shortcode_unautop'  );
add_filter( 'tkmnineteen_the_content', 'do_shortcode'       );
 ...
 */
function tkmnineteen_first_term( $taxonomy = 'category', $field = false, $post_id = false ) {
 ...
 */
function tkmnineteen_class( $base_classes, $optional_class, $conditional ) {
 ...
 */
function tkmnineteen_column_class( $classes = array(), $current = false, $join = true ) {
 ...
 */
function tkmnineteen_bg_image_style( $image_id = false, $image_size = 'full' ) {
	if( !empty( $image_id ) )
 ...
 *
 */
function tkmnineteen_icon( $atts = array() ) {
 ...
 *
 */
function tkmnineteen_has_action( $hook ) {
	ob_start();
	do_action( $hook );
	$output = ob_get_clean();
	return !empty( $output );
