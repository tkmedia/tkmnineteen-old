...
**/
  ...
  */
 function tkmnineteen_dont_update_theme( $r, $url ) {
 ...
 add_filter( 'http_request_args', 'tkmnineteen_dont_update_theme', 5, 2 );
/**
 * Dequeue jQuery Migrate
 *
 */
function tkmnineteen_dequeue_jquery_migrate( &$scripts ){
	if( !is_admin() ) {
		$scripts->remove( 'jquery');
		$scripts->add( 'jquery', false, array( 'jquery-core' ), '1.10.2' );
	}
}
add_filter( 'wp_default_scripts', 'tkmnineteen_dequeue_jquery_migrate' );
/**
 * Header Meta Tags
 *
 */
function tkmnineteen_header_meta_tags() {
...
add_action( 'wp_head', 'tkmnineteen_header_meta_tags' );
/**
 * Clean Nav Menu Classes
 *
 */
function tkmnineteen_clean_nav_menu_classes( $classes ) {
	if( ! is_array( $classes ) )
		return $classes;
...
add_filter( 'nav_menu_css_class', 'tkmnineteen_clean_nav_menu_classes', 5 );
/**
 * Staff comment class
 *
 */
function tkmnineteen_staff_comment_class( $classes, $class, $comment_id, $comment, $post_id ) {
	if( empty( $comment->user_id ) )
		return $classes;
	$staff_roles = array( 'comment_manager', 'author', 'editor', 'administrator' );
	$staff_roles = apply_filters( 'tkmnineteen_staff_roles', $staff_roles );
	$user = get_userdata( $comment->user_id );
	if( !empty( array_intersect( $user->roles, $staff_roles ) ) )
		$classes[] = 'staff';
	return $classes;
}
add_filter( 'comment_class', 'tkmnineteen_staff_comment_class', 10, 5 );
/**
 * Remove avatars from comment list
 *
 */
function tkmnineteen_remove_avatars_from_comments( $avatar ) {
	global $in_comment_loop;
	return $in_comment_loop ? '' : $avatar;
}
add_filter( 'get_avatar', 'tkmnineteen_remove_avatars_from_comments' );
/**
 * Archive Title, remove prefix
 *
 */
function tkmnineteen_archive_title_remove_prefix( $title ) {
...
add_filter( 'get_the_archive_title', 'tkmnineteen_archive_title_remove_prefix' );
/**
 * Comment form, button class
 *
 */
function tkmnineteen_comment_form_button_class( $args ) {
	$args['class_submit'] = 'submit wp-block-button__link';
	return $args;
}
add_filter( 'comment_form_defaults', 'tkmnineteen_comment_form_button_class' );
/**
 * Excerpt More
 *
 */
function tkmnineteen_excerpt_more() {
	return '&hellip;';
}
add_filter( 'excerpt_more', 'tkmnineteen_excerpt_more' );
/**
